...
def force_require_2fa(modeladmin, request, queryset):
    ...
    messages.success(request, f"Đã bật ép buộc 2FA cho {updated} user được chọn.")
...
def disable_2fa(modeladmin, request, queryset):
    ...
    for user in queryset:
        if user.is_2fa_enabled:
            ...
            updated_count += 1
    messages.success(request, f"Đã tắt 2FA cho {updated_count} user được chọn.")
...
def unlock_otp(modeladmin, request, queryset):
    ...
    for user in queryset:
        ...
        updated += 1
    messages.success(request, f"Đã mở khoá OTP cho {updated} user.")
...
def force_password_reset(modeladmin, request, queryset):
    ...
    messages.success(request, f"Đã ép {updated} user phải đổi mật khẩu ở lần đăng nhập kế tiếp.")
...
class SecurityPolicyAdmin(admin.ModelAdmin):
    ...
    def force_all_users_require_2fa(self, request, queryset):
        ...
        messages.success(request, f"Đã ép {updated} user phải bật 2FA...")
